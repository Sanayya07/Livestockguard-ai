...
import { Camera, Mic, Footprints, Thermometer, FileText, CheckCircle2, Loader2, Cpu, Sparkles } from 'lucide-react';
...
const steps: AnalysisStep[] = [
  {
    ...
    icon: Camera,
    details: ['Detecting body condition score', 'Analysing posture and gait', 'Scanning for eye/nose discharge', 'Evaluating coat quality'],
  },
  {
    id: 'audio',
    ...
    icon: Mic,
    details: ['Measuring respiratory rate', 'Detecting coughing events', 'Identifying wheezing sounds', 'Classifying vocalisations'],
  },
  {
    id: 'behaviour',
    ...
    icon: Footprints,
    details: ['Comparing activity to baseline', 'Estimating rumination time', 'Analysing feed intake patterns', 'Checking social interaction'],
  },
  {
    id: 'farm',
    ...
    icon: Thermometer,
    details: ['Evaluating temperature stress', 'Assessing humidity impact', 'Checking ventilation adequacy', 'Contextualising activity setting'],
  },
  {
    id: 'report',
    ...
    icon: FileText,
    details: ['Weighting indicator severity', 'Calculating risk score', 'Generating explanation', 'Compiling recommendations'],
  },
];
...
export function ProcessingPage({ onNavigate }: ProcessingPageProps) {
  ...
  useEffect(() => {
    if (currentStep >= steps.length) {
      const timer = setTimeout(() => onNavigate('report'), 800);
      ...
      <Card className="overflow-hidden">
        <div className="relative bg-gradient-to-br from-brand-600 to-brand-800 px-6 py-8 text-center text-white">
          ...
          <div className="relative">
            ...
            <h2 className="mt-4 font-display text-xl font-bold">Analysis in Progress</h2>
            <p className="mt-1 text-sm text-brand-100">
              Analysing Animal LG-017 · Cattle · 4 years
            </p>
          </div>
        </div>
        <div className="px-6 py-4">
          <div className="mb-2 flex items-center justify-between text-sm">
            <span className="font-medium text-ink-600">Overall progress</span>
            <span className="font-bold text-brand-600">{Math.round(progress)}%</span>
          </div>
        ...
        {steps.map((step, idx) => {
          ...
          return (
            ...
            >
              <div className="flex items-start gap-4 p-5">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <h3 className="font-display text-sm font-semibold text-ink-900">{step.label}</h3>
                    {isCompleted && (
                  ...
                  {(isActive || isCompleted) && (
                    <ul className="mt-3 space-y-1.5">
                      {step.details.map((detail, di) => {
                        const detailDone = isCompleted || (isActive && di < Math.floor((Date.now() / 600) % step.details.length));
                        return (
                          <li
                            key={di}
                            className={cn(
                              'flex items-center gap-2 text-xs transition-all',
                              isCompleted ? 'text-ink-500' : detailDone ? 'text-ink-600' : 'text-ink-400',
                            )}
                            style={{ animationDelay: `${di * 200}ms` }}
                          >
                            {isCompleted ? (
                              <CheckCircle2 className="h-3.5 w-3.5 text-success-500" />
                            ) : (
                              <span className={cn('h-1.5 w-1.5 rounded-full', detailDone ? 'bg-brand-500' : 'bg-ink-300')} />
                            )}
                            {detail}
                          </li>
                        );
              ...
              {/* Animated scan line for active step */}
              {isActive && (
